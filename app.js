...
(function (global) {
  ...
  function initGlobalPomodoroWidget() {
    ...
    // d'onglet du site, sans avoir à retourner sur l'onglet Pomodoro.
    function playCustom() {
      currentKind = 'custom';
  ...
  function playSchoolChimeSound(repeat) {
    try {
      ...
      var sequenceLength = playSequence(0);
      // Pour la fin d'un cycle complet : on rejoue le carillon une seconde
      // fois juste après la fin de la résonance de la première passe.
      if (repeat) playSequence(sequenceLength + 0.55);
